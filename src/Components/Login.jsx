...
function Login() {
  ...
  const handleLogin = () => {
    const existingUser = JSON.parse(localStorage.getItem('users')); // Changed to 'users' to match your registration logic.

    if (!existingUser) {
      alert('No account found. Please register first.');
    } else if (existingUser.some(user => user.email === email && user.password === password)) {
        ...
        <div className="flex flex-col md:flex-row items-center justify-center bg-white max-w-4xl w-full rounded-lg p-6 space-y-6 md:space-y-0 md:space-x-8 shadow-lg">
          ...
          <div className="flex flex-col space-y-4 w-full md:w-1/2 py-8">
            ...
            <div className="text-center mx-4">
              <Link to='/register' className="text-[#848484] text-sm">Have no account yet?</Link>
            </div>
